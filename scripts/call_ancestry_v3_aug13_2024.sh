...
awk '{print $1,$2}' temporary_files/"$2"_anc.fam | split  -l 300 -d - temporary_files/"$2"_anc_
...
do 
    ...
    then
     echo "#!/bin/bash
        #SBATCH -N ancestry_determination
...
" > temporary_files/"$2"_anc_"$j".snpweights_parfile 
            ...
            echo "python2.7 $6 --par temporary_files/"$2"_anc_"$j".snpweights_parfile &" >> temporary_files/ancjob"$job"_"$2".pbs
           fi
        done
...
for i in  $(seq 1 1 $totjobs)
 ...
 sbatch --time=0:30:00 temporary_files/ancjob"$i"_"$2".pbs #for bigger jobs
